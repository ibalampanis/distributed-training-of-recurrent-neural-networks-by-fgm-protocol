...
public:
    ...
    void testAssignConst() {
        ...
        sk = 3.0;
        for (auto x = begin(sk); x != end(sk); x++) TS_ASSERT_EQUALS(*x, 3.0);

        sk *= 5;
        for (auto x = begin(sk); x != end(sk); x++) TS_ASSERT_EQUALS(*x, 15.0);

        sk = 0;
        for (auto x = begin(sk); x != end(sk); x++) TS_ASSERT_EQUALS(*x, 0.0);
        TS_ASSERT_EQUALS(sk.norm2_squared(), 0.0);
    }
