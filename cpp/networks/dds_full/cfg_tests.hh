...
public:
    ...
    void test_parse_url() {
        ...
        check_purl("file:a/v/hello.cc?x=1,y=hello world",
                   ...
                   "a/v/hello.cc",
                   vmap{{"x", "1"},
                        {"y", "hello world"}}, default_open_mode, default_text_format
        );

        check_purl("file:/hello.cc?open_mode=truncate",
                   "file",
                   "/hello.cc",
                   vmap{{"open_mode", "truncate"}}, open_mode::truncate, default_text_format
        );

        check_purl("hdf5:/hello.cc", "hdf5", "/hello.cc", vmap{}, default_open_mode, default_text_format
        );

        check_purl("hdf5:/hello.cc?open_mode=append,group=/foo/bar",
                   "hdf5",
                   "/hello.cc",
                   vmap{{"open_mode", "append"},
                        {"group",     "/foo/bar"}}, open_mode::append, default_text_format
        );

        check_purl("hdf5:hello?format=csvtab", "hdf5", "hello", vmap{{"format", "csvtab"}}, default_open_mode,
                   text_format::csvtab
        );
        check_purl("stdout:", "stdout", "", vmap{}, default_open_mode, default_text_format
        );

        check_purl("foo:", "foo", "", vmap{}, default_open_mode, default_text_format
        );
    }
